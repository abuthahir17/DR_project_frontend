...
function Dashboard() {
  ...
  const getUrgencyIndicator = (severityIndex) => {
    const urgencyLevels = {
      0: { label: 'ROUTINE', color: '#10b981', icon: '✓' },
      1: { label: 'SCHEDULED', color: '#f59e0b', icon: '⚠' },
      2: { label: 'URGENT', color: '#f97316', icon: '⚡' },
      3: { label: 'CRITICAL', color: '#ef4444', icon: '🚨' }
    };
    return urgencyLevels[severityIndex] || urgencyLevels[0];
  };
  ...
  const getRiskAssessment = (severityIndex, age) => {
    const baseRisk = severityIndex * 25;
    const ageRisk = age > 60 ? 10 : age > 50 ? 5 : 0;
    return Math.min(baseRisk + ageRisk, 100);
  };

  const getFollowUpInterval = (severityIndex) => {
    const intervals = {
      0: '12 months',
      1: '6-9 months',
      2: '3-4 months',
    ...
    return intervals[severityIndex] || '12 months';
  };

  const getProgressionRisk = (severityIndex) => {
    const risks = {
      0: { level: 'LOW', percentage: '5%', color: '#10b981' },
      1: { level: 'MODERATE', percentage: '25%', color: '#f59e0b' },
      2: { level: 'HIGH', percentage: '50%', color: '#f97316' },
      3: { level: 'VERY HIGH', percentage: '75%', color: '#ef4444' }
    };
    return risks[severityIndex] || risks[0];
  };

  const getManagementStrategy = (severityIndex) => {
    const strategies = {
      0: ['Annual comprehensive eye examination', 'Maintain optimal glycemic control', 'Regular BP monitoring'],
      1: ['Enhanced monitoring every 6 months', 'Strict diabetes management', 'Lipid profile assessment'],
      2: ['Quarterly ophthalmology visits', 'Consider laser therapy', 'Anti-VEGF evaluation'],
      3: ['Immediate specialist consultation', 'Urgent treatment planning', 'Weekly monitoring']
    };
    return strategies[severityIndex] || strategies[0];
  };

  const getPreventiveGuidelines = () => {
    return {
      glycemic: ['Target HbA1c < 7%', 'Regular glucose monitoring', 'Medication adherence'],
      lifestyle: ['Balanced diet', '30 min exercise daily', 'Smoking cessation'],
      monitoring: ['Annual eye exams', 'BP checks', 'Kidney function tests']
    };
  };

  const calculateDRProgression = (currentSeverity, durationYears) => {
    const progressionRate = currentSeverity === 0 ? 0.05 : currentSeverity === 1 ? 0.15 : currentSeverity === 2 ? 0.30 : 0.50;
    const projectedRisk = Math.min(progressionRate * durationYears * 10, 95);
    return {
      oneYear: Math.round(projectedRisk * 0.2),
      threeYears: Math.round(projectedRisk * 0.6),
      fiveYears: Math.round(projectedRisk)
    };
  };

  const getTreatmentProtocol = (severityIndex) => {
    const protocols = {
      0: {
        immediate: 'No treatment required',
        monitoring: 'Annual dilated fundus examination',
        prevention: 'Maintain optimal diabetes control'
      },
      1: {
        immediate: 'Enhanced surveillance',
        monitoring: 'Semi-annual retinal screening',
        prevention: 'Intensify glycemic management'
      },
      2: {
        immediate: 'Ophthalmology referral within 1 month',
        monitoring: 'Quarterly retinal assessment',
        prevention: 'Consider focal laser or anti-VEGF'
      },
      3: {
        immediate: 'URGENT: Specialist consultation within 48-72 hours',
        monitoring: 'Weekly to bi-weekly assessment',
        prevention: 'Pan-retinal photocoagulation + Anti-VEGF therapy'
      }
    };
    return protocols[severityIndex] || protocols[0];
  };

  const getDietaryPlan = () => {
    return {
      recommended: [
  ...
  const getExerciseRecommendations = () => {
    return {
      aerobic: ['Brisk walking 30 min daily', 'Swimming', 'Cycling'],
      strength: ['Light resistance training 2-3x/week', 'Bodyweight exercises'],
      flexibility: ['Yoga', 'Stretching exercises'],
      precautions: ['Avoid high-impact activities', 'Monitor blood sugar before/after', 'Stay hydrated']
    };
  };

  const getComplicationWarnings = (severityIndex) => {
    const complications = {
      0: [],
      1: ['Risk of progression to PDR', 'Possible macular edema development'],
      2: ['Macular edema', 'Vitreous hemorrhage risk', 'Retinal detachment possibility'],
      3: ['SEVERE: Vision loss imminent', 'Vitreous hemorrhage', 'Tractional retinal detachment', 'Neovascular glaucoma']
    };
    return complications[severityIndex] || [];
  };

  const getSystemicAssessment = () => {
    return {
      cardiovascular: ['Blood pressure monitoring', 'Lipid profile', 'ECG if indicated'],
      renal: ['Serum creatinine', 'eGFR calculation', 'Urine albumin-to-creatinine ratio'],
      metabolic: ['HbA1c every 3 months', 'Fasting glucose', 'Postprandial glucose'],
      neurological: ['Peripheral neuropathy screening', 'Autonomic function assessment']
    };
  };

  const getQualityOfLifeImpact = (severityIndex) => {
    const impacts = {
      0: 'Minimal impact on daily activities',
      1: 'Mild visual disturbances possible',
      2: 'Moderate impact on vision-dependent tasks',
      3: 'Severe impact - assistance may be required'
    };
    return impacts[severityIndex] || 'Assessment required';
  };

  const getInsuranceCoverage = () => {
    return {
      screening: 'Typically covered annually for diabetics',
      treatment: 'Laser therapy and anti-VEGF injections usually covered',
      surgery: 'Vitrectomy covered if medically necessary',
      medications: 'Most diabetes medications covered',
      note: 'Coverage varies by insurance plan - verify with provider'
    };
  };
      ...
      <main style={{
        ...
        {activeTab === 'register' && (
            ...
            >
                ...
                {/* DOWNLOAD BUTTON SECTION - COMPACT DESIGN */}
                    ...
                    <div style={{
                        ...
                        <div>
                            <p style={{
                                ...
                                <span style={{fontSize: '20px'}}>📄</span> Official Medical Report Ready
                            </p>
                            <p style={{
                                margin: '6px 0 0',
                                color: '#64748b',
